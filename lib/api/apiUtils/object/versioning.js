...
function _deleteNullVersionMD(bucketName, objKey, options, log, cb) {
    ...
    return _getVersionLocation(bucketName, objKey, options, log,
        (err, nullDataToDelete) => {
            if (err) {
                log.warning('could not find null version metadata', {
                    error: err,
...
export function versioningPreprocessing(bucketName, bucketMD, objectKey,
    ...
    // NOTE: Currently we do not accept receive requests with PUTs to
    // specific version ID, but this may change after implementing
    // georeplication.
    } else if (mstIsNull) {
        ...
        return _getVersionLocation(bucketName, objectKey,
            { versionId: reqVersionId }, log, (err, dataToDelete) => {
                if (err && err === 'NoSuchVersion') {
                    // no data to delete
                    return callback(null, options);
                } else if (err) {
                    return callback(errors.InternalError);
